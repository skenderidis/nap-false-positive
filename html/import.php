<?php
   ...
   function get_policies_gitlab($git_fqdn, $project, $token, $id, $path, $branch, $uuid) {
		$headers = array(
         ...
         $httpcode = curl_getinfo($curl, CURLINFO_HTTP_CODE);
            ...
            foreach ($result as $key)
				{
					if ($key["type"] == "blob" && (strpos($key['name'], ".json") !== false || strpos($key['name'], ".yaml") !== false || strpos($key['name'], ".yml") !== false))
					{
   ...
   function get_policies_gitea($git_fqdn, $project, $token, $path, $branch, $uuid) {
		$headers = array(
			'Content-Type: application/json',
			'Accept: application/json, text/javascript, */*; ',
			'PRIVATE-TOKEN: ' . $token
         ...
         $httpcode = curl_getinfo($curl, CURLINFO_HTTP_CODE);
            ...
            foreach ($result as $key)
				{
					if ($key["type"] == "file" && (strpos($key['name'], ".json") !== false || strpos($key['name'], ".yaml") !== false || strpos($key['name'], ".yml") !== false))
					{
   ...
   if ($type=="gitlab")
   {
      $result = get_policies_gitlab($git_fqdn, $project, $token, $id, $path, $branch, $uuid);
   ...
   if ($type=="gitea")
   {
      $result = get_policies_gitea($git_fqdn, $project, $token, $path, $branch, $uuid);
      if ($result["status"]==1)
      {
         $policies = "var policies = " . json_encode($result["msg"])  . ";";       
      }
